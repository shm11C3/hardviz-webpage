...
**公開タイミングの訂正**: このページは対象リリースと同時に公開する。以下の草案の「予告」表現は初稿の記録であり、最終ページはリリース済みの紹介として作成した。対象バージョンはユーザー指定のv1.11.0。草案作成時点では公開日未確定のため未記載だったが、v1.11.0は2026-09-26に公開され、日英の特設ページとChangelogをこのPRに追加した。

## リリース同時公開の確認事項

草案作成時に記録した公開前チェック。v1.11.0の公開後、日英Changelogを作成し、特設ページからChangelog、Changelog一覧から特設ページの相互リンクを追加した。
